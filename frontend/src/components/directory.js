...
const Directory = () => {
    ...
    const updateAgent = async(ag, index) => {
        setSelectedAgent(ag)
        updateAgentInfo.sid = index
        setEditAgent(!editAgent)
    }

    const updateAgentFunc = () => {
        data[updateAgentInfo.sid].name = updateAgentInfo.name
        data[updateAgentInfo.sid].email = updateAgentInfo.email
        data[updateAgentInfo.sid].forwarding_number = updateAgentInfo.forwardingNumber
                    ...
                    {editAgent !== false && (
                        <div className="selectedAgentContainer">
                            ...
                            <input className="editAgentInput" value={updateAgentInfo.name} onChange={(e) => {
                                setUpdateAgentInfo({name:e.target.value, forwardingNumber:updateAgentInfo.forwardingNumber, email:updateAgentInfo.email, sid: updateAgentInfo.sid})
                            }}></input>
                            </div> 
                            <div>
                            Number: 
                            <input className="editAgentInput" value={updateAgentInfo.forwardingNumber} onChange={(e) => {
                                setUpdateAgentInfo({name:updateAgentInfo.name, forwardingNumber:e.target.value, email:updateAgentInfo.email, sid: updateAgentInfo.sid})
                            }}></input>
                            </div>
                            <div>
                            Email: 
                            <input className="editAgentInput" value={updateAgentInfo.email} onChange={(e) => {
                                setUpdateAgentInfo({name:updateAgentInfo.name, forwardingNumber:updateAgentInfo.forwardingNumber, email:e.target.value, sid: updateAgentInfo.sid})
                            }}></input>
                            </div>
                            <div>
